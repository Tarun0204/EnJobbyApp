...
const FiltersGroup = props => {
  const renderLocationTypesList = () => {
    ...
    return locationTypesList.map(eachLocationType => {
      const updatelocationTypesList = () =>
        updateLocationTypesChecked(eachLocationType.locationTypeId)

      return (
        <li className="fliters-list-item" key={eachLocationType.locationTypeId}>
          <input
            type="checkbox"
            className="checkbox-input"
            id={eachLocationType.locationTypeId}
            onChange={updatelocationTypesList}
          />
          <label
            htmlFor={eachLocationType.locationTypeId}
            className="filter-label"
          >
            {eachLocationType.label}
          </label>
        </li>
      )
    })
  ...
  const renderLocationTypes = () => (
    <>
      <h1 className="filter-heading">Location Type</h1>
