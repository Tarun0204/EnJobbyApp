...
class Jobs extends Component {
  ...
  updateLocationTypesChecked = locationFilterId => {
    const {locationTypesChecked} = this.state
    let updatedLocationList = locationTypesChecked
    if (locationTypesChecked.includes(locationFilterId)) {
      updatedLocationList = locationTypesChecked.filter(
        eachLocation => eachLocation !== locationFilterId,
      )
    } else {
      updatedLocationList = [...updatedLocationList, locationFilterId]
    }
  ...
  getJobs = async () => {
    ...
    const locationTypes = locationTypesChecked.join(',')
    const jwtToken = Cookies.get('jwt_token')
    const apiUrl = `https://apis.ccbp.in/jobs?employment_type=${employTypes}&minimum_package=${activeSalaryRangeId}&search=${searchInput}&location_types=${locationTypes}`
